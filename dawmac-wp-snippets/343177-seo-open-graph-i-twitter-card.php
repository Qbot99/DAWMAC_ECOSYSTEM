...
if ( ! function_exists( 'dawmac_og_default_image' ) ) {
	...
	/**
	 * Opis produktu z parametrow felgi i ceny — do podgladow linkow.
	 * Zwraca pusty string dla wszystkiego, co nie jest produktem
	 * albo nie ma zadnych parametrow.
	 */
	function dawmac_og_opis_felgi( $post_id ) {
		...
		// Etykiety trafiają do podglądu linku u klienta, więc z polskimi znakami.
		$pola = array(
			'Średnica'  => 'pa_srednica',
			'Szerokość' => 'pa_szerokosc',
			'Rozstaw'   => 'pa_rozstaw',
			'ET'        => 'pa_et',
		);
		...
		foreach ( $pola as $etykieta => $atrybut ) {
			$wartosc = $product->get_attribute( $atrybut );
			if ( '' !== trim( (string) $wartosc ) ) {
				$czesci[] = $etykieta . ': ' . $wartosc;
			}
		}
		...
		if ( '' !== (string) $cena && null !== $cena ) {
			$czesci[] = 'Cena: ' . wp_strip_all_tags( wc_price( $cena ) );
		}

		return dawmac_og_trim( implode( ' | ', $czesci ) );
	}
	...
	function dawmac_og_data() {
		...
		if ( is_front_page() || is_home() ) {
			...
			/*
			 * Dla felg opis budujemy z parametrów i ceny, a nie z treści wpisu.
			 * W podglądzie na WhatsAppie i Facebooku "19\" 8.5J ET42 5x112 |
			 * 3180 zł" mówi więcej niż pierwsze zdanie opisu produktu.
			 * Ta logika przyszła ze snippetu #328701, który wypisywał własne
			 * tagi og: obok tych poniżej — stąd brały się duplikaty.
			 */
			$zParametrow = dawmac_og_opis_felgi( $post_id );
